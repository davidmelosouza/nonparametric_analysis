function [D] = KDEMAP_ROI_MISE(DATA,X,Y,nROI,setup)
% X=xgrid;
% Y=ygrid{1};
...
subplot(4,1,1); MAP_ROI(DATA,DC,Y,DATA.sg.RoI.iRoI);
subplot(4,1,1); xlabel('Variável Aleatória'); %set(gca,'Fontsize',12);
setup.TYPE.ROI = 'prob';
[DATA] = datasetGenMix(setup);
DATA.sg.RoI.Xaxis = linspace(0,1,length(DATA.sg.RoI.Xaxis));
subplot(4,1,2);MAP_ROI(DATA,DC,Y,DATA.sg.RoI.iRoI);
subplot(4,1,2); xlabel('Probabilidade');%set(gca,'Fontsize',12);
setup.TYPE.ROI = 'deriv';
[DATA] = datasetGenMix(setup);
DATA.sg.RoI.Xaxis = linspace(0,1,length(DATA.sg.RoI.Xaxis));
subplot(4,1,3);MAP_ROI(DATA,DC,Y,DATA.sg.RoI.iRoI);
subplot(4,1,3); xlabel('1ª Derivada');%set(gca,'Fontsize',12);
