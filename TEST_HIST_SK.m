...
for name = {'GGD','Logn'}
    ...
    for ir=1:length(range.SK)
        ...
        for im=1:ntmax
            ...
            disp(['[HIST][PCT][' num2str(ir/length(range.SK)) ']NAME[' name{1} ']IT[' num2str(im) ']TRUTH[' num2str(bin.truth) ']TIME[' num2str(toc) ']'])
            
        end
    end
%     pause
    save([pwd '\HIST\HIST[SK]DIST[' name{1} ']EVT[' num2str(nEVT) ']'],'BIN','AREA');
end