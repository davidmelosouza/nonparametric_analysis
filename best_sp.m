function [bsp,ei,es,i,ef,fa] = best_sp(ef,fa,datae,dataj)
    ...
    [am_ef,bm_ef]=binofit(ef*length(datae),length(datae));
    eim_ef=am_ef-bm_ef(1);
    esm_ef=bm_ef(2)-am_ef;
    [am_fa,bm_fa]=binofit((1-fa)*length(dataj),length(dataj));
