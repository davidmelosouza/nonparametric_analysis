function [X,pdf,A,L2] = KDEfast_fixed_ep_RANGE(setup,DATA,H,nPoint)
...
pdf=zeros(1,nPoint);
for j=1:Div
    pdf(1,(1+(nPoint/Div)*(j-1)):(nPoint/Div)*(j))=(((1/n)*sum((repmat((Hi.^(-1/2)),nPoint/Div,1).*Knep(repmat((Hi.^(-1/2)),nPoint/Div,1).*((repmat(X((1+(nPoint/Div)*(j-1)):(nPoint/Div)*(j)),length(data),1)')-repmat(data,nPoint/Div,1)))),2))');
end
...
% disp(['[Epanechikov]AREA_EST[' num2str(AE) ']AREA_PDF[' num2str(AP) ']'] );
%% Calculo dos Erros:
