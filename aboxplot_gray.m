...
outMarker = '+';
outMarkerSize = 3;
outMarkerEdgeColor = [0 0 0];
outMarkerFaceColor = [0 0 0];
% alpha = 0.1;
cmap = [];
colorrev = 0;
colorgrd = 'blue_down';
...
for j=1:d
    ...
    for i=1:n
        ...
        line([i-widthl/2+(2*j-1)*widthl/(2*d) i-widthl/2+(2*j-1)*widthl/(2*d)],[fu fl],...
            'Color',colors{j,1},'LineStyle',':','HitTest','off','Parent',hgg(j));
        line([i-widthl/2+(2*j-1-widthe)*widthl/(2*d) i-widthl/2+(2*j-1+widthe)*widthl/(2*d)],[fu fu],...
            'Color',colors{j,1},'HitTest','off','Parent',hgg(j));
        ...
        if q3 > q1
            rectangle('Position',[i-widthl/2+(2*j-1-widths)*widthl/(2*d) q1 widths*widthl/d q3-q1],...
                'EdgeColor',[0.9 0.9 0.9],'FaceColor',[0.9 0.9 0.9],'HitTest','off','Parent',hgg(j));
        end
        
%            % Plot fence
%         line([i-widthl/2+(2*j-1)*widthl/(2*d) i-widthl/2+(2*j-1)*widthl/(2*d)],[fu fl],...
%             'Color',[0.9 0.9 0.9],'LineStyle',':','HitTest','off','Parent',hgg(j));
%         line([i-widthl/2+(2*j-1-widthe)*widthl/(2*d) i-widthl/2+(2*j-1+widthe)*widthl/(2*d)],[fu fu],...
%             'Color',[0.9 0.9 0.9],'HitTest','off','Parent',hgg(j));
%         line([i-widthl/2+(2*j-1-widthe)*widthl/(2*d) i-widthl/2+(2*j-1+widthe)*widthl/(2*d)],[fl fl],...
%             'Color',[0.9 0.9 0.9],'HitTest','off','Parent',hgg(j));
%         
%         % Plot quantile
...
%         end
        ...
        plot(i-widthl/2+(2*j-1)*widthl/(2*d),q2,...
            'LineStyle','none',...
            'Marker','s',...
            'Markersize',5,...
        ...
        XL(i) =i-widthl/2+(2*j-1)*widthl/(2*d);
        
        hold on
        if (i==1) || (i==2) || (i==3) || (i==4) || (i==5) || (i==6) || (i==7) || (i==8) || (i==9) || (i==10) || (i==11) || (i==12) || (i==13) || (i==14) || (i==15) || (i==16) 
            if (j == d)
                  vx = i-widthl/2+(2*j-1)*widthl/(2*d);  
                  plot([vx+0.085*widthl vx+0.085*widthl],[min(min(minY)) max(max(maxY))*1.5],':','Color',[0.85 0.85 0.85],'Linewidth',2)
            end
        end
