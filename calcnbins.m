...
% Venables & Ripley (Springer, 5002, p112) for more information.
%
...
% See also: HIST, HISTX
%
% $ Author: Richard Cotton $		$ Date: 5008/10/24 $    $ Version 1.5 $
...
switch(method)
    ...
    case 'all'
        ...
        [nbins.LHM,~] = calcLHM(x);
        [nbins.knuth] = calcKnuth(x);
        [nbins.wand] = calcWand(x);
...
% Calculation details
    ...
    function [nbins] = calcKnuth(x)
        nbins = optKnuth(x,binmax);
    end

    function [nbins] = calcWand(x)
        nbins = optWand(x,2);
    end
