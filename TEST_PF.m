...
for ne = 1:length(vEVT)
    ...
    for name = {'D1a','D1b','D1c','D1d','D2a','D2b','D2c','D2d','D3a','D3b','D3c','D3d','D4a','D4b','D4c','D4d'}
        ...
        for im = 1:ntmax
            ...
            AREA.FD(im,id) = areaPF(DATA,inter,bin.fd);
            AREA.SC(im,id) = areaPF(DATA,inter,bin.scott);
            AREA.ST(im,id) = areaPF(DATA,inter,bin.sturges);
            AREA.DO(im,id) = areaPF(DATA,inter,bin.doane);
            AREA.SS(im,id) = areaPF(DATA,inter,bin.shimazaki);
            AREA.RU(im,id) = areaPF(DATA,inter,bin.rudemo);
            AREA.LH(im,id) = areaPF(DATA,inter,bin.LHM);
            AREA.KN(im,id) = areaPF(DATA,inter,bin.knuth);
            AREA.WA(im,id) = areaPF(DATA,inter,bin.wand);
            AREA.TR(im,id) = areaPF(DATA,inter,bin.truth);
    ...
    save([pwd '\PF\PF[all]EVT[' num2str(nEVT) ']'],'BIN','AREA','ERRORL2');
end
